...
afltables_stats_temp <- fitzRoy::fetch_player_stats_afltables(season = NULL, round_number = NULL)
afltables_stats <- data.frame(lapply(afltables_stats_temp, set_lists_to_chars), stringsAsFactors = F)
write.csv(afltables_stats,"data//afl_players_stats.csv", row.names = FALSE)
